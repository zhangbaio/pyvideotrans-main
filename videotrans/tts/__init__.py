...
from videotrans.tts._doubaoclone import DoubaoCloneTTS
from videotrans.tts._qwenttsclone import QwenttsCloneTTS
...
TTS_API = 32
CAMB_TTS = 33
DOUBAO_CLONE_TTS = 34   # 字节火山引擎 声音复刻 2.0
QWEN_CLONE_TTS = 35     # 阿里百炼 DashScope CosyVoice-v2 声音克隆
...
_ID_NAME_DICT = {
    ...
    DOUBAO_CLONE_TTS: "VolcEngine Clone (声音复刻2.0)",
    QWEN_CLONE_TTS: "Qwen Clone (CosyVoice-v2/DashScope)",
}
...
def is_input_api(tts_type: int = None, return_str=False):
    ...
    if tts_type == GLM_TTS and not params.get('zhipu_key'):
        ...
        zhipuai_win.openwin()
        return False
    if tts_type == DOUBAO_CLONE_TTS and (
            not params.get('volcenginetts_appid', '') or not params.get('volcenginetts_access', '')):
        if return_str:
            return "Please configure volcenginetts_appid / volcenginetts_access for VolcEngine Clone first."
        from videotrans.winform import volcenginetts as volcengine_win
        volcengine_win.openwin()
        return False
    if tts_type == QWEN_CLONE_TTS:
        missing = []
        if not params.get('qwentts_key', ''):
            missing.append('qwentts_key')
        for k in ('oss_bucket', 'oss_endpoint', 'oss_access_key_id', 'oss_access_key_secret'):
            if not params.get(k, ''):
                missing.append(k)
        if missing:
            if return_str:
                return f"Please configure {', '.join(missing)} for Qwen Clone (DashScope + OSS)."
            return False
    return True
...
def run(*, queue_tts=None, language=None, uuid=None, play=False, is_test=False, tts_type=0,is_cuda=False) -> None:
    ...
    elif tts_type == DOUBAO_CLONE_TTS:
        DoubaoCloneTTS(**kwargs).run()
    elif tts_type == QWEN_CLONE_TTS:
        QwenttsCloneTTS(**kwargs).run()
