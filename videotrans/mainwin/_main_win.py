...
from videotrans.mainwin._presets import PresetManager
...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def _bind_signal(self):
        ...
        # L1 预设菜单 (参数模板一键加载/另存)
        # 延迟到事件循环启动后再安装, 避免在 __init__ 里动 menuBar 触发 Qt 崩溃
        def _install_preset_menu():
            try:
                self.preset_manager = PresetManager(self)
                self.preset_manager.install_menu()
            except Exception:
                logger.exception('install preset menu failed')
        QTimer.singleShot(500, _install_preset_menu)
